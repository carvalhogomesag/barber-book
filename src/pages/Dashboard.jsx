...
export function Dashboard() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (prof) {
        setProfile(prof);
        if (prof.settings?.businessHours) setBusinessHours(prev => ({ ...prev, ...prof.settings.businessHours }));
      }
  ...
  const getNowInTimezone = () => new Date(new Date().toLocaleString("en-US", { timeZone: timezone }));
  ...
  const handleSaveBlock = async (formData) => {
    ...
    try {
      await addBlockedTime({
        startTime: fullStartDateStr, duration: parseInt(blockDuration), notes: blockNotes,
        recurrence: blockRecurrence, occurrences: parseInt(blockOccurrences)
      });
      await refreshAppointments();
  ...
  return (
    <AppLayout>
      {/* 
          ESTRUTURA DE TRAVA (FLEX-1 MIN-H-0)
          Isso obriga o calendário a ter um limite de altura e habilitar o Scroll.
      */}
      <div className="flex-1 flex flex-col min-h-0 w-full">
        
        {/* HEADER */}
        <div className="flex items-center justify-between mb-4 bg-white p-4 rounded-[24px] border border-schedy-border shadow-sm shrink-0">
